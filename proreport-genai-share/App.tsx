...
import { SubSection, ReportData, SelectionState, ReferenceData, ReferenceFile, SectionRefs, ReferenceGroup, ReferenceType, Project, ProjectVersion, Chapter } from './types';
...
type AppView = 'cover' | 'dashboard' | 'workspace' | 'outline-editor';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const savedLibrary = localStorage.getItem('pro_report_library');
        if (savedLibrary) {
            try { 
                const parsed = JSON.parse(savedLibrary);
                const migrated = parsed.map((f: ReferenceFile) => {
            ...
            } catch(e) { setLibraryFiles([]); }
        }
        const savedGroups = localStorage.getItem('pro_report_custom_groups');
        if (savedGroups) {
            try { setCustomGroups(JSON.parse(savedGroups)); } catch (e) { setCustomGroups([]); }
        }
        const savedProjects = localStorage.getItem('pro_report_projects');
        if (savedProjects) {
    ...
    useEffect(() => { 
        try {
            localStorage.setItem('pro_report_library', JSON.stringify(libraryFiles)); 
        } catch (e) {
            console.error("Storage limit reached");
            alert("本地存储空间已满，部分文件可能未保存。建议清理旧文件或使用较小的文件。");
        }
    }, [libraryFiles]);
    useEffect(() => { localStorage.setItem('pro_report_custom_groups', JSON.stringify(customGroups)); }, [customGroups]);
    useEffect(() => {
        ...
        setProjects(prevProjects => {
            ...
            localStorage.setItem('pro_report_projects', JSON.stringify(updated));
            return updated;
        });
    }, [reportData, referenceData, projectInfo, projectName, projectOutline, isProjectInfoLocked, currentProjectId]);
    ...
    const startCreateProject = () => { setNewProjectNameInput(""); setNewProjectDescInput(""); setIsOutlineCreatorOpen(true); };
    const confirmCreateProject = (useCustomOutline: boolean) => {
        const projectId = 'proj-' + Date.now();
        const baseProject: Project = { id: projectId, name: newProjectNameInput.trim() || '未命名项目 ' + new Date().toLocaleDateString(), description: newProjectDescInput, outline: STANDARD_TEMPLATE, reportData: {}, referenceData: {}, isLocked: false, lastModified: Date.now(), history: [] };
        if (useCustomOutline) {
            setEditingOutline([{ title: "第一章 概述", subsections: [{ id: "1-1", title: "第一节 项目概况", req: "请在此处输入大纲要求..." }] }]);
            setCurrentProjectId(projectId);
            setProjectName(baseProject.name);
            setProjectInfo(baseProject.description);
    ...
    const finishOutlineEditing = () => {
        const newProject: Project = { id: currentProjectId!, name: projectName, description: projectInfo, outline: editingOutline, reportData: {}, referenceData: {}, isLocked: false, lastModified: Date.now(), history: [] };
    ...
    const handleDeleteProject = (e: React.MouseEvent, id: string) => {
        ...
        localStorage.setItem('pro_report_projects', JSON.stringify(remaining));
                ...
                <div className="z-20 text-center flex flex-col items-center select-none">
                    ...
                    >
                        <span className="relative z-10 flex items-center gap-3">
                            INITIALIZE SYSTEM <Icons.ChevronRight />
                        </span>
